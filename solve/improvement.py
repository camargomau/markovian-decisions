    ...
    for state in process.states:
        ...
        for target_state in process.states:
            if target_state == state:
                equation.append(
                    discount*process.transition[decision_in_state][state][target_state]-1)
            else:
                equation.append(
                    discount*process.transition[decision_in_state][state][target_state])
        ...
        for decision in process.decisions:
            if process.decision_applicability[decision][state]:
                ...
                for target_state in process.states:
                    ...
                    if discount == 1:
                        if target_state == state:
                            cost += (process.transition[decision][state]
                                     [target_state]-1)*system_solution[target_state]
                        else:
                            cost += (process.transition[decision][state]
                                     [target_state])*system_solution[target_state]
                    else:
...
def main(process, discounted):
    ...
    global table
    table = PrettyTable()
    field_names = ["iter."] + [f"V{i}" for i in process.states] + \
