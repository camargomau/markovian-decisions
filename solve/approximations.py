...
def find_v_policy(iter, process, discount, previous_v):
    ...
    for state in process.states:
        ...
        if iter == 1:
            to_compare = {decision: cost for decision, cost in enumerate(process.costs[state]) if cost is not None}
...
def successive(process, discount, max_iter, tol):
    ...
    while iter <= max_iter:
        current_v, current_policy = find_v_policy(iter, process, discount, previous_v)
        ...
        if iter != 1:
            ...
            all_within_tolerance = all(abs(v - previous_v[i]) < tol for i, v in enumerate(current_v))
...
def main(process):
    ...
    tol = inp.number("• Introduzca la tolerancia: ", number_type="f", min_value=0)

    # Obtener una _aproximación_ a la política óptima
    approximated_optimal_policy, within_tolerance = successive(process, discount, max_iter, tol)

    print(f"\n{table}")

    print("\n(tolerancia alcanzada)") if within_tolerance else print("\n(número máximo de iteraciones alcanzado)")
    print(f"\nLa aproximación obtenida a la política óptima es {approximated_optimal_policy}")
